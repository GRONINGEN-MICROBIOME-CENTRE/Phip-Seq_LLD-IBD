...
pheno = read.table("pheno_data/HLA_4analysis2.txt")
dataMat_T = read.table("pheno_data/probes_4analysis2.txt")
...
result = foreach(i = args[2]:args[3],.combine = rbind)%:%
  foreach(j = 1:ncol(dataMat_T),.combine = rbind)%do%{
    ...
    data.frame(HLA = colnames(pheno)[i],
...
write.table(result,paste0("results.probes.HLA/result.batch",args[1],".txt"),sep="\t",quote = F,row.names=F)
